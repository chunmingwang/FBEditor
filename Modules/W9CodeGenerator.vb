...
Public Module W9CodeGenerator
    ...
    Public Function GenerateMultiFormCode(project As W9FormProject) As String
        If project Is Nothing OrElse project.Forms.Count = 0 Then Return ""

        ' Single form — delegate to original generator
        If project.Forms.Count = 1 Then
            Return GenerateCode(project.Forms(0))
        End If

        ' ----- Multi-form generation -----
        Dim sb As New StringBuilder()
        Dim main = project.MainForm

        ' File header
        sb.AppendLine("' ============================================================================")
        sb.AppendLine("' " & main.FormTitle & " - FreeBASIC + Window9 (Multi-Form)")
        sb.AppendLine("' ============================================================================")
        sb.AppendLine("' Generated by FBEditor v5.1.0 Visual Form Designer")
        sb.AppendLine("' Date: " & DateTime.Now.ToString("yyyy-MM-dd HH:mm"))
        sb.AppendLine("' Forms: " & project.Forms.Count)
        For Each f In project.Forms
            sb.AppendLine("'   " & f.VarName & " — " & f.FormTitle &
                          If(f.FormType = W9FormType.MainForm, " (main)", " (child)"))
        Next
        sb.AppendLine("' ============================================================================")
        ...
        Dim anyResize = project.Forms.Any(Function(f) f.ProportionalResize)
        If anyResize Then
        ...
        Dim anyMenus = project.Forms.Any(Function(f) f.MenuItems.Count > 0)
        If anyMenus Then
            sb.AppendLine("' --- Menu IDs ---")
            sb.AppendLine("Enum MenuID")
            sb.AppendLine("  miFirst = " & main.MenuEnumStart)
            For Each f In project.Forms
                For Each topMenu In f.MenuItems
        ...
        ' Shortcut enum
        Dim anyShortcuts = project.Forms.Any(Function(f) f.HasKeyboardShortcut)
        If anyShortcuts Then
        ...
        If anyMenus Then
            ...
            sb.AppendLine()
        End If
        Dim anyTimer = project.Forms.Any(Function(f) f.HasTimer)
        If anyTimer Then
        ...
        For Each f In project.Forms
            If f.ProportionalResize AndAlso f.HandleResize AndAlso f.Gadgets.Count > 0 Then
                Dim subName = GetResizeSubName(f)
                sb.AppendLine("' ============================================================================")
                ...
                For Each g In f.Gadgets
                    ...
                    If g.GadgetType = W9GadgetType.Editor Then
                        If g.IsReadOnly Then sb.AppendLine("  ReadOnlyEditor(" & g.EnumName & ", 1)")
                        If g.WordWrap Then sb.AppendLine("  SetTransferTextLineEditorGadget(" & g.EnumName & ", 1)")
                    End If
                Next
                sb.AppendLine()
        ...
        ' This follows your pattern: hSecondForm created, gadgets added, then hMainForm created
        For Each f In project.Forms
            If f.FormType = W9FormType.ChildForm Then
        ...
        If main.ProportionalResize Then
        ...
        Dim resizeForms = project.Forms.Where(Function(f) f.ProportionalResize AndAlso f.HandleResize AndAlso f.Gadgets.Count > 0).ToList()
        If resizeForms.Count > 0 Then
            sb.AppendLine("    Case EventSize")
            For Each f In resizeForms
        ...
        ' Menu events
        If anyMenus OrElse anyShortcuts Then
            sb.AppendLine("    Case EventMenu")
            sb.AppendLine("        Select Case EventNumber")
            If anyShortcuts Then
                Dim defaultBtn = main.Gadgets.Find(Function(g) g.ID = main.DefaultButtonID)
        ...
        Dim childForms = project.Forms.Where(Function(f) f.FormType = W9FormType.ChildForm).ToList()
        If childForms.Count > 0 Then
            For Each f In childForms
                sb.AppendLine("        If EventHwnd() = " & f.VarName & " Then")
                If f.HideOnClose Then
    ...
    Private Function GetResizeSubName(f As W9FormDesign) As String
        If f.FormType = W9FormType.MainForm Then Return "size_change"
